...
public class OfferBoardGui {
    ...
    private void confirmAccept(Offer offer) {
        ...
        confirm.fillRect(18, 26, services.fillerItem());
    ...
    private void decide(long offerId, boolean accept) {
        ...
        boolean admin = viewer.hasPermission("ah.admin");
        OfferDecisionService.Result r = accept
                ? (admin
                        ? services.decisions().acceptAsAdmin(viewer.getUniqueId(), offerId)
                        : services.decisions().accept(viewer.getUniqueId(), offerId))
                : (admin
                        ? services.decisions().rejectAsAdmin(viewer.getUniqueId(), offerId)
